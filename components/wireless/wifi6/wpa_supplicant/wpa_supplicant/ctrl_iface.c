...
#elif !defined(__CYGWIN__) && !defined(CONFIG_NATIVE_WINDOWS) && !defined(CONFIG_MACSW)
		...
		if (ssid) {
			...
			case WPAS_MODE_INFRA:
				ret = os_snprintf(pos, end - pos,
						  "mode=station\n");
				break;
#ifndef CONFIG_ONLY_STA_AP_MODE
			case WPAS_MODE_IBSS:
				ret = os_snprintf(pos, end - pos,
						  "mode=IBSS\n");
				break;
...
#else /* CONFIG_MACSW */
#ifdef CONFIG_P2P
static int p2p_ctrl_peers(struct wpa_supplicant *wpa_s, char *cmd,
			  char *buf, size_t buflen)
{
	u8 addr[ETH_ALEN], *addr_ptr;
	int next, res, discovered = 0, res_info;
	const struct p2p_peer_info *info;
	char buf_info[512], *pos, *end;

	if (os_strcmp(cmd, "discovered") == 0)
		discovered = 1;

	if (!wpa_s->global->p2p)
		return -1;

	// Initialise to get the FIRST peer
	addr_ptr = NULL;
	next = 0;
	...
	while (1) {
		...
		memcpy(addr, info->p2p_device_addr, sizeof(addr));

		res_info = p2p_get_peer_info_txt(info, &buf_info[0], sizeof(buf_info));
		if (res_info < 0)
			return pos - buf;

		if (!discovered || strstr(buf_info, "[PROBE_REQ_ONLY]") == NULL) {
			res = os_snprintf(pos, end - pos, MACSTR "\n",MAC2STR(addr));
			if (os_snprintf_error(end - pos, res))
				return pos - buf;
			pos += res;
...
static int wpa_supplicant_ctrl_iface_get_status(
       struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
       int id;
       size_t res;
       ...
       /* cmd: "<network id>" */
       id = atoi(cmd);
       wpa_printf(MSG_EXCESSIVE, "CTRL_IFACE: GET_NETWORK id=%d",
                  id);
...
static int wpa_supplicant_ctrl_iface_ap_sta_del(
       struct wpa_supplicant *wpa_s, char *cmd)
{
    char *mac_addr;
    u8 addr[ETH_ALEN];
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (0) {
		...
		reply_len = wpa_supplicant_ctrl_iface_get_network(
			wpa_s, buf + 12, reply, reply_size);
#endif
	} else if (os_strncmp(buf, "GET_STATUS ", 11) == 0) {
        reply_len = wpa_supplicant_ctrl_iface_get_status(
            wpa_s, buf + 11, reply, reply_size);
	} else if (os_strncmp(buf, "AP_STA_DEL ", 11) == 0) {
		if (wpa_supplicant_ctrl_iface_ap_sta_del(wpa_s, buf + 11))
			reply_len = -1;
#ifndef CONFIG_NO_CONFIG_WRITE
	} else if (os_strncmp(buf, "DUP_NETWORK ", 12) == 0) {
		if (wpa_supplicant_ctrl_iface_dup_network(wpa_s, buf + 12,
		...
		reply_len = p2p_ctrl_peer(wpa_s, buf + 9, reply,
					  reply_size);
	} else if (os_strcmp(buf, "P2P_PEERS") == 0) {
		reply_len = p2p_ctrl_peers(wpa_s, "", reply,
					   reply_size);
	} else if (os_strncmp(buf, "P2P_PEERS ", 10) == 0) {
		reply_len = p2p_ctrl_peers(wpa_s, buf + 10, reply,
					   reply_size);
	} else if (os_strncmp(buf, "P2P_SET ", 8) == 0) {
		if (p2p_ctrl_set(wpa_s, buf + 8) < 0)
			reply_len = -1;
