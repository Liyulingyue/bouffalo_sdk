...
void wpas_conf_ap_vht(struct wpa_supplicant *wpa_s,
			      ...
			      struct hostapd_config *conf)
{
	...
	if (wpa_s->hw.modes) {
		...
		if (mode && is_6ghz_freq(ssid->frequency) &&
			...
			conf->ht_no_overlap_check = ssid->ht_no_overlap_check;
			     ...
			     struct wpa_ssid *ssid)
{
	...
	if (params.uapsd > 0) {
		conf->bss[0]->wmm_enabled = !ssid->disable_wmm;
		conf->bss[0]->wmm_uapsd = !ssid->disable_wmm;
	}
